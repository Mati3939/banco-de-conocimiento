registerModule({id:'lineal-factor-integrante',title:'Lineal de primer orden y factor integrante',unidad:'I',semanas:[2],evaluacion:['control-1','certamen-1'],
  ...
  build(sec){
    ...
    c1.append(el('p',{},'Reconocerla: si la ecuación viene como $a(x)y\'+b(x)y=c(x)$, hay que ',el('b',{},'dividir por $a(x)$'),' antes de leer $P(x)=b(x)/a(x)$ y $Q(x)=c(x)/a(x)$ — es el error más común del método. Ejemplo (se resuelve completo más abajo): $(1+t)\\dfrac{dy}{dt}+2y=6t^2$ se lleva a forma estándar dividiendo por $(1+t)$:'));
    ...
    Pasos(c2,[
      ...
      {tex:'\\dfrac{d\\mu}{\\mu}=P(x)\\,dx\\ \\Longrightarrow\\ \\ln|\\mu|=\\int P(x)\\,dx',nota:'La condición sobre μ es, a su vez, una EDO separable — se resuelve con la misma técnica del tema anterior.'},
    ...
    c3.append(el('p',{class:'fuente'},'Fuente: síntesis de la derivación de la card anterior — elaboración propia.'));
    sec.append(c3);
    ...
    c5.append(el('p',{class:'fuente'},'Fuente: ejercicio de elaboración propia, siguiendo la receta de las cards anteriores de este tema (no proviene de un PDF de Canvas ni de una pauta oficial).'));
    sec.append(c5);
